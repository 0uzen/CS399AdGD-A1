...
public class CharacterController2D : MonoBehaviour
{
    ...
    private void Update()
    {
		...
		if (jump)
        {
			if (m_Grounded)
            {
				m_Rigidbody2D.AddForce(Vector2.up * 600, ForceMode2D.Force);
				m_Grounded = false;
				enableAirJump = true;
            }
    ...
    {
        if (collision.gameObject.layer == m_WhatIsGround)
        {
			m_Grounded = true;
		...
		if (collision.gameObject.tag == SpikeTag)
        {
			if (m_FacingRight)
            {
				m_Rigidbody2D.AddForce(Vector2.left * 5000, ForceMode2D.Force);
            }
            else
            {
				m_Rigidbody2D.AddForce(Vector2.right * 5000, ForceMode2D.Force);
            }
